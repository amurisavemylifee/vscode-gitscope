...
История ведётся через переименования (`git log --follow`): на коммите, где файл
сменил имя, она не обрывается, а карточка показывает прежний путь. Версии
подгружаются страницами по мере прокрутки.
...
- **История листается курсором**, а не `--skip`: пропущенные коммиты `--follow`
  не разбирает, и если в пропуск попадал коммит переименования, история под ним
  терялась целиком. Следующая страница берётся от родителя последнего коммита.
